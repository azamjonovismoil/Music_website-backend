"""
sync_engine.py
Audio → LRC lyrics synchronization.
Primary:  WhisperX (word-level alignment)
Fallback: faster-whisper (segment-level alignment)
"""
...
def _sync_whisperx(audio_path: str, lyrics_lines: list, model_size: str = "base") -> dict:
    ...
    try:
        ...
        aligned = whisperx.align(
            transcription["segments"], align_model, metadata,
...
def _align_words_to_lines(lines: list, words: list) -> list:
    """Match lyric lines to word-level timestamps (WhisperX output)."""
    synced = []
    cursor = 0
    ...
    for line in lines:
        ...
        if best and best["ratio"] > 0.1 and best["start_idx"] < total:
            ...
            synced.append({
                "time": round(s, 2), "start": round(s, 2),
                "end": round(e, 2), "text": clean_text(line),
                "confidence": round(best["ratio"], 2), "words": [],
            })
...
def _align_segments_to_lines(lines: list, segments: list) -> list:
    """Match lyric lines to transcript segments (faster-whisper output)."""
    synced = []
    used = set()
    ...
    for line in lines:
        ...
        if best_idx is not None and best_score >= 0.12:
            ...
            used.add(best_idx)
            synced.append({
                "time": round(seg["start"], 2), "start": round(seg["start"], 2),
                "end": round(seg["end"], 2), "text": clean_line,
                "confidence": round(best_score, 2), "words": [],
            })
...
def _fill_gaps(lines: list, synced: list, duration: float) -> list:
    """Fill any unmatched lines with evenly spaced timestamps."""
        ...
        return [
            {
                ...
                "text": line, "confidence": 0.2, "words": [],
            }
    ...
    for i, line in enumerate(clean_lines):
        if line not in synced_texts:
            ...
            final.append({
                ...
                "text": line, "confidence": 0.15, "words": [],
            })
...
def generate_sync_from_lyrics(audio_path: str, lyrics_text: str, model_size: str = "base") -> dict:
    """
    Main entry point.
    Returns dict with: language, duration, syncedLyricsRaw, syncedLyrics, backend
    """
    ...
    # Try WhisperX first (better word alignment)
    if BACKEND == "whisperx":
        try:
    ...
    if FASTER_AVAILABLE:
        try:
            ...
            raise RuntimeError(f"Both backends failed. Last error: {e}") from e
